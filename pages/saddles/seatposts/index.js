...
import { productSaddlePageQuery } from '../../../lib/api';
  ...
  return(
    <div>
      <>
        <Layout>
          <Container>
            <h1 className="main-headline text-6xl font-bold mb-8 pt-10 mb-8">seatposts.</h1>
            ...
            {
              morePosts.length > 0 && <ProductPage posts={morePosts} uidName="seatposts" apiName="Saddles" />
            }
          </Container>
        </Layout>
      </>
...
export async function getStaticProps({ preview = false, previewData }) {
  const allPosts = await productSaddlePageQuery(previewData)
  return {
